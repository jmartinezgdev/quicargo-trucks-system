...
const ItemTruck: React.FC<Truck> = (truck: Truck) => {
    return (
        <div className="truck-item bg-white shadow-sm p-4 rounded-md hover:shadow-lg">
            <div>
                <span>{LABELS.truck.MODEL}</span>
                <span>{truck.model}</span>
            </div>
            <div>
                <span>{LABELS.truck.YEAR}</span>
                <span>{truck.year}</span>
            </div>
            <div>
                <span>{LABELS.truck.LICENCE_PLATE}</span>
                <span>{truck.licensePlate}</span>
            </div>
            <div>
                <span>{LABELS.truck.CURRENT_KM}</span>
                <span>{truck.currentKm}</span>
            </div>
            <div>
                <span>{LABELS.truck.MAXIMUM_LOAD_KG}</span>
                <span>{truck.maximumLoadKg}</span>
            </div>
            <div>
                <span>{LABELS.truck.FUEL_TYPE}:</span>
